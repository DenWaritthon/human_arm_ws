...
movej_buttons = [
    ToggleButton(SCREEN_WIDTH // 4 - button_width // 2, section_y, button_width, button_height, "Move by Q", active=True),
    ToggleButton(3 * SCREEN_WIDTH // 4 - button_width, section_y, button_width * 1.5, button_height, "Target J Ref by World")
]
...
movel_move_button = Button(5 * SCREEN_WIDTH // 6 - 150, 600, 300, 50, "Move")  # Positioned in the Joint Effort section
...
while running:
    ...
    if toggle_buttons[0].active:  # MoveJ Mode
        ...
        for button in movej_buttons:
            button.draw(screen)

        # ---- MoveJ - Move by Q ----
        if movej_buttons[0].active:  # "Move by Q" is selected
            # Draw scrollbars
        ...
        # ---- MoveJ - Target J Ref by World ----
        elif movej_buttons[1].active:  # "Target J Ref by World" is selected
        ...
        force_title_rect = force_title.get_rect(center=(SCREEN_WIDTH // 2, header_height + 150))
        screen.blit(force_title, force_title_rect)
        ...
        force_x_label = SMALL_FONT.render("X:", True, BLACK)
        screen.blit(force_x_label, (force_x_input.rect.x - 30, force_x_input.rect.y + 10))

        force_y_label = SMALL_FONT.render("Y:", True, BLACK)
        screen.blit(force_y_label, (force_y_input.rect.x - 30, force_y_input.rect.y + 10))

        force_z_label = SMALL_FONT.render("Z:", True, BLACK)
        screen.blit(force_z_label, (force_z_input.rect.x - 30, force_z_input.rect.y + 10))

        # ---- 3rd Section: Joint Effort ----
        # Display text values for Joint Efforts
        for i, effort in enumerate(joint_effort_values):
            effort_surface = SMALL_FONT.render(effort, True, BLACK)
            effort_rect = effort_surface.get_rect(topleft=(5 * SCREEN_WIDTH // 6 - 100, 250 +100 + i  * 30))
            screen.blit(effort_surface, effort_rect)

        # Draw the Move button for MoveL in Joint Effort Section
    ...
    for event in pygame.event.get():
        ...
        # Handle MoveJ section toggle buttons
        if toggle_buttons[0].active:
            for button in movej_buttons:
                if button.handle_event(event):
                    for other_button in movej_buttons:
                        other_button.active = False
                    button.active = True

            # Handle scrollbars in "Move by Q"
            if movej_buttons[0].active:
                for scrollbar in scrollbars:
                    scrollbar.handle_event(event)

                # Handle the Move button
                if move_button.handle_event(event):
                    # Print joint values when Move button is clicked
                    values = [f"{scrollbar.label}: {scrollbar.get_value():.2f}" for scrollbar in scrollbars]
                    print("Move by Q Joint Values:")
                    for value in values:
                        print(value)

            # Handle inputs for "Target J Ref by World"
            if movej_buttons[1].active:
                ...
                # Handle the Move button
                if target_move_button.handle_event(event):
        ...
        if toggle_buttons[1].active:  # MoveL Mode
            ...
            # Handle MoveL Move Button
            if movel_move_button.handle_event(event):
    ...
    if toggle_buttons[0].active and movej_buttons[1].active:
